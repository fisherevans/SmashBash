...
import com.fisherevans.smash_bash.game.states.command.commandObjects.*;
...
public class CommandState extends OverlayState implements TypingState {
    ...
    public CommandState(SmashBashState overlayedState) {
    ...
    public void enterState(GameContainer container, StateBasedGame game) throws SlickException {
        acceptInput = true;
        while(_console.size()*font.getLineHeight() > SmashBash.height())
            _console.remove(0);
    }
    ...
    public void render(Graphics graphics) throws SlickException {
        ...
        for(int id = _console.size()-1;id >= 0;id--) {
            ...
            graphics.drawString(text.getText(), 10, SmashBash.height() - font.getLineHeight()*(3+_console.size()-id));
        }

        graphics.setColor(Color.white);
        graphics.drawRect(8, SmashBash.height() - font.getLineHeight()*2 - 2, SmashBash.width()-16, font.getLineHeight()+4);
        graphics.drawString(_inputLine, 10, SmashBash.height() - font.getLineHeight()*2);

        if(flash) {
            float x = font.getWidth(_inputLine.substring(0, _cursor));
            graphics.drawLine(10+x, SmashBash.height() - font.getLineHeight()*2-2, 10+x, SmashBash.height() - font.getLineHeight()+2);
        }
    }
    ...
    @Override
    public int getID() {
        return SmashBash.STATE_COMMAND;
    }
